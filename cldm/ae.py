...
class SecretEncoder_new(nn.Module):
    def __init__(self, base_res=16, resolution=64) -> None:
        super().__init__()
    ...
    def forward(self, secret_image):
        secret_image = thf.interpolate(secret_image, size=(self.base_res, self.base_res), mode='bilinear')
        secret_image = secret_image.permute(0, 3, 1, 2)  # swap dimensions to match network input
        output = self.secret_scaler(secret_image)
        return output
...
class SecretEncoder_new(nn.Module):
    def __init__(self, embed_dim, ddconfig, ckpt_path=None,
                 ignore_keys=[], colorize_nlabels=None,
                 monitor=None, ema_decay=None):
        super().__init__()
        self.embed_dim = embed_dim
    ...
    def forward(self, x):
        x = self.encoder(x)  # Process input image
        x = self.quant_conv(x)  # Convert input image to embedded representation
        return x
...
class SecretEncoder7(nn.Module):
    def __init__(self, base_res=16, resolution=64) -> None:
        ...
        self.feature_extractor = nn.Sequential(*list(resnet.children())[:-2])  # 移除最后的 FC 层
        self.secret_scaler = nn.Sequential(
            nn.Conv2d(2048, 3, kernel_size=1),  # ResNet50 的输出是 2048 通道
            nn.Upsample(scale_factor=(2 ** (log_resolution - log_base), 2 ** (log_resolution - log_base))),
            # chx16x16 -> chx256x256
            zero_module(conv_nd(2, 3, 3, 3, padding=1))
        )  # secret len -> ch x res x res
...
class ControlAE1(pl.LightningModule):
    def __init__(self,
                 first_stage_key,
                 first_stage_config,
                 control_key,
